...
export function CreateScheduledEventModal(props: CreateScheduledEventProps) {
  ...
  const group = createFormGroup({
    ...
    coverImage: createFormControl<string | File[] | null>(
      existingEvent()?.coverImageURL ?? null,
    ),
  ...
  const voiceChannels = createMemo(() =>
    (server()?.channels ?? []).filter((ch) => ch.isVoice),
  );
  ...
  async function onSubmit() {
    ...
    try {
      ...
      if (group.controls.coverImage.isDirty) {
        if (!group.controls.coverImage.value) {
          // User removed the image — only relevant for editing
        } else if (Array.isArray(group.controls.coverImage.value)) {
          data.cover_image = await client().uploadFile(
            "banners",
            group.controls.coverImage.value[0],
      ...
      <Switch>
        ...
        <Match when={step() === 1}>
          <Column gap="md">
            ...
            <Form2.FileInput
              control={group.controls.coverImage}
              accept="image/*"
              label={t`Cover Image`}
              imageRounded={false}
              imageJustify={false}
        ...
        <Match when={step() === 2}>
          <Column gap="md">
            <Show when={group.controls.coverImage.value}>
              <PreviewCoverImage
                src={
                  Array.isArray(group.controls.coverImage.value)
                    ? URL.createObjectURL(group.controls.coverImage.value[0])
                    : (group.controls.coverImage.value as string)
                }
