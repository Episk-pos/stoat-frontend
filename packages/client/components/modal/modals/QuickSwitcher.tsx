...
import { Avatar, Dialog } from "@revolt/ui";
...
export function QuickSwitcherModal(
  ...
  const defaultResults = createMemo(() => {
    ...
    // Recent conversations (DMs/Groups sorted by activity)
    let recent = state.ordering
      .orderedConversations(c)
      .slice(0, 8)
      .map(channelToResult)
      .filter(Boolean) as ResultItem[];

    // Fallback: if ordering is empty, get DMs/Groups directly
    if (!recent.length) {
      recent = c.channels
        .toList()
    ...
    // Recent server channels (regardless of unread)
    const recentChannels = c.channels
      .toList()
      .filter(
        (ch) =>
          ch.type !== "DirectMessage" &&
      ...
      .map(channelToResult)
      .filter(Boolean) as ResultItem[];
    if (recentChannels.length) results.push({ label: "Recent Channels", items: recentChannels });

    // Collect IDs already shown
    const shownIds = new Set(results.flatMap((r) => r.items.map((i) => i.id)));

    // Unread channels (excluding already-shown)
    const unread = c.channels
      .toList()
      .filter((ch) => ch.unread && ch.type !== "DirectMessage" && ch.type !== "Group" && !shownIds.has(ch.id))
      .slice(0, 8)
      .map(channelToResult)
      .filter(Boolean) as ResultItem[];
