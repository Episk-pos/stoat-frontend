...
export const ServerEventDetail: Component = () => {
  ...
  const canManageEvents = createMemo(
    () =>
      server()?.havePermission("ManageEvents"),
  );

  function formatEntityType(ev: ScheduledEvent): string {
        ...
        <Switch>
          ...
          <Match when={query.isError}>
            <Column align justify grow>
              ...
              <span><Trans>Failed to load event.</Trans></span>
            </Column>
          </Match>
          <Match when={!event()}>
            <Column align justify grow>
              <Symbol size={48}>event_busy</Symbol>
              <span><Trans>Event not found.</Trans></span>
            </Column>
          </Match>
          <Match when={event()}>
            {(ev) => {
              const [interested, setInterested] = createSignal(false);
              ...
              async function toggleInterest() {
                ...
                try {
                  if (interested()) {
                    await ev().removeInterest();
                    setInterested(false);
                  } else {
                    await ev().addInterest();
                    setInterested(true);
                  }
                } catch {
                  // Silently handle — optimistic UI
                } finally {
                  setToggling(false);
                }
              }
              ...
              <EventDetailContent>
                ...
                <DetailBody>
                  ...
                  <DetailSection>
                    ...
                    <DetailValue>
                      <Switch fallback={ev().location ?? t`Unknown`}>
                  ...
                  <DetailSection>
                    ...
                    <DetailValue>
                      <Plural value={ev().interestedCount} one="# person" other="# people" />
                    </DetailValue>
                  </DetailSection>

                  <Show when={ev().status === "Scheduled" || ev().status === "Active"}>
                    <Row gap="md" wrap>
                      <Button
                        size="xs"
                        variant={interested() ? "filled" : "tonal"}
                        onPress={toggleInterest}
                        isDisabled={toggling()}
                      >
                        <Symbol size={16}>{interested() ? "star" : "star_outline"}</Symbol>
                        {interested() ? <Trans>Interested</Trans> : <Trans>Interested?</Trans>}
                      </Button>
                      <Button
                        size="xs"
                        variant="tonal"
                        onPress={() => {
                          navigator.clipboard.writeText(
                            `${window.location.origin}/server/${params.server}/events/${ev().id}`
                          );
                        }}
                      >
                      ...
                      <Button
                        ...
                        onPress={() =>
                          openModal({
                            ...
                            event: ev(),
                            cb: () => navigate(`/server/${params.server}/events`),
                          })
                        }
                      >
...
function StatusBadge(props: { status: ScheduledEventStatus }) {
  ...
  function formatStatus(status: ScheduledEventStatus): string {
    switch (status) {
      case "Scheduled":
        return t`Scheduled`;
      case "Active":
