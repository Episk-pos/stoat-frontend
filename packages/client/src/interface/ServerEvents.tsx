        ...
        <Switch>
          ...
          <Match when={query.isError}>
            <Column align justify grow>
              ...
              <span><Trans>Failed to load events.</Trans></span>
            </Column>
          </Match>
          <Match when={currentList().length === 0}>
            <EmptyState>
              ...
              <span>
                <Switch fallback={<Trans>No events to show.</Trans>}>
                  <Match when={tab() === "upcoming"}>
                    <Trans>No upcoming events scheduled.</Trans>
                  </Match>
                  <Match when={tab() === "active"}>
                    <Trans>No events are currently active.</Trans>
                  </Match>
                  <Match when={tab() === "past"}>
                    <Trans>No past events.</Trans>
                  </Match>
                </Switch>
              </span>
            </EmptyState>
...
function EntityBadge(props: { event: ScheduledEvent }) {
  ...
  return (
    <Row align gap="xs" class={entityBadgeStyle}>
      ...
      <span>
        <Switch fallback={props.event.location ?? formatEntityType(props.event)}>
      ...
      <CardContent>
        ...
        <Show when={props.event.interestedCount > 0}>
          <Row align gap="xs" class={metaRow}>
            ...
            <span>
              <Plural
                value={props.event.interestedCount}
